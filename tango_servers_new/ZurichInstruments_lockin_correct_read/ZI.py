...
"""
ZI
Zurich Instruments MFLI lock-in amplifier TANGO server.
Modern tango.server port of the original PyTango.Device_4Impl version.
...
class ZI(Device):
    """
    ZI MFLI lock-in amplifier.

    **Properties:**
    - DeviceId: ZI device serial (e.g. 'dev4855')
    ...
    DeviceId = device_property(
        dtype='str',
        default_value='dev4855',
        doc="ZI device serial (e.g. 'dev4855')"
    )
    ...
    def _path(self, suffix):
        """Build a full ZI node path: '/dev4855/<suffix>'."""
    ...
    # ---- demodulator readouts (populated by ThreadZI) ------------------
...
def main(args=None, **kwargs):
    return run((ZI,), args=args, **kwargs)
