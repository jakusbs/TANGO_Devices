...
class AttoDRYThread(threading.Thread):
    """
    Listener thread for AttoDRY UDP packets.
    """
    ...
    def run(self):
        while self._running:
            ...
            try:
                ...
                # If sending fails (socket closed, etc.), exit immediately
                break

            # Wait for a valid "ReadA..." packet
            try:
                data, addr = self.p.s.recvfrom(self.bufsize)
            except socket.timeout:
                # No packet within timeout; loop again (and re-send)
            ...
            try:
                ...
                # If decoding fails, skip this cycle
                time.sleep(self.interval)
                continue
            ...
            # Now unpack each field by its letter markers:
            ...
            # If any expected marker is missing, skip parsing this packet
            ...
            try:
                # Convert (int or float) and store on the parent device
            ...
            # Update the parent devices internal fields
            ...
            # Update the Tango cache attributes so read_* methods will see them
