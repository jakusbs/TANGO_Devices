...
void Socket::always_executed_hook()
{
	...
	// if code is placed here uncomment the INFO_STREAM above
...
void Socket::write(Tango::DevString argin)
{
	...
	try
	{
		*client_socket << argin;
		*client_socket << "\r\n";
		DEBUG_STREAM << "Socket::Write()  - writing " << argin << endl;	
	}
	catch ( SocketException& e )
	{
		TangoSys_MemStream	tms;
		...
		ERROR_STREAM << "Socket::Write()  - " << e.description() << " writing " << argin << endl;
	}
 ...
 */
//--------------------------------------------------------
void Socket::write_line(Tango::DevString argin)
{
	DEBUG_STREAM << "Socket::WriteLine()  - " << device_name << endl;
	/*----- PROTECTED REGION ID(Socket::write_line) ENABLED START -----*/
	...
	/*----- PROTECTED REGION END -----*/	//	Socket::write_line
}
//--------------------------------------------------------
/**
 ...
 */
//--------------------------------------------------------
Tango::DevString Socket::write_read(Tango::DevString argin)
{
	Tango::DevString argout;
	DEBUG_STREAM << "Socket::WriteRead()  - " << device_name << endl;
	/*----- PROTECTED REGION ID(Socket::write_read) ENABLED START -----*/
	...
	try
	{
		*client_socket << argin;
		//*client_socket << "\r\n";		
		*client_socket << "\n";
		DEBUG_STREAM << "Socket::WriteRead()  - writing " << argin << endl;
	}
	catch ( SocketException& e )
	{
		TangoSys_MemStream	tms;
		tms << "Socket::write(" << argin << ")";
		manage_exception(e.description(), tms.str());
		ERROR_STREAM << "Socket::WriteRead()  - Write error: " << e.description() << endl;
	}

	/*// read until end of input (see read())
	bool	empty = false;
	string	reply("");
	while (!empty)
	{
	...
	// read until end of line (\n or 0x0a or ASCII 10) adapted from readln()
	// changed back from readuntil \r or <CR> or ASCII 13 (for DG645): was not working
	string reply;
	//char terminator = '\r';
	
	try
	{
		client_socket->readln(reply, readtimeout); // using standard \n terminator
		//client_socket->readuntil(reply, &terminator, readtimeout);
		DEBUG_STREAM << "Socket::WriteRead()  - reading " << reply << endl;
	}
	catch ( SocketException& e )
	{
		//manage_exception(e.description(), "Socket::readln()");
		reply = "";
		ERROR_STREAM << "Socket::WriteRead()  - Read error or empty: " << e.description() << endl;	
	}
	argout = CORBA::string_dup(reply.c_str());

	/*----- PROTECTED REGION END -----*/	//	Socket::write_read
	return argout;
}
//--------------------------------------------------------
/**
 ...
 */
//--------------------------------------------------------
Tango::DevString Socket::write_read_zero(Tango::DevString argin)
{
	Tango::DevString argout;
	DEBUG_STREAM << "Socket::WriteReadZero()  - " << device_name << endl;
	/*----- PROTECTED REGION ID(Socket::write_read_zero) ENABLED START -----*/
	
	// writing adapted from write_line
	check_connection();
	try
	{
		*client_socket << argin;
		*client_socket << "\0";
		DEBUG_STREAM << "Socket::WriteReadZero()  - writing " << argin << endl;
	}
	catch ( SocketException& e )
	{
		TangoSys_MemStream	tms;
		tms << "Socket::write(" << argin << ")";
		manage_exception(e.description(), tms.str());
		ERROR_STREAM << "Socket::WriteReadZero()  - Write error: " << e.description() << endl;
	}
	...
	try
	{
		/* first working version with suspected small memory leak
        char terminator = '\0';
		
		// read reply until ASCII 0
        client_socket->readuntil(reply, &terminator, readtimeout);
		//Tango::DevString argout = CORBA::string_dup(reply.c_str()); // done below!!
              
        // read everything what is remaining (reads the two signal recovery bytes)
        read();
        */
        
        /*
        // second version (not working because strings read may be empty)
        //read everything available at once
        string replyZero = read();
        
        // inspect answer, should be ASCII 0 terminated plus 2 trailing bytes (status, to discard)
        string::size_type pos;
        pos = reply.find('\0');
        cout << "Reply length = " << replyZero.length() << endl;
        // now cut rest
        reply = replyZero.substr(0,pos);
        cout << "  New length = " << reply.length() << endl;
        */
        
        // third version: like first version, read() copied from above
        char terminator = '\0';
	...
	{
		//manage_exception(e.description(), "Socket::write_read_zero()");
		reply = "";
		ERROR_STREAM << "Socket::WriteReadZero()  - Read error or empty: " << e.description() << endl;	
	}
    
	argout = CORBA::string_dup(reply.c_str());
    
	/*----- PROTECTED REGION END -----*/	//	Socket::write_read_zero
	return argout;
}
//--------------------------------------------------------
/**
 ...
 */
//--------------------------------------------------------
Tango::DevString Socket::read_char()
{
	Tango::DevString argout;
	DEBUG_STREAM << "Socket::ReadChar()  - " << device_name << endl;
	/*----- PROTECTED REGION ID(Socket::read_char) ENABLED START -----*/
	
    // read single char (byte) from socket
    
    char reply = '\0';
	try
	{
	    client_socket->readchar(reply, readtimeout);
    }
	catch ( SocketException& e )
	{
		ERROR_STREAM << "Socket::ReadChar()  - Read error or empty: " << e.description() << endl;	
	}
	//cout << " got char response: " << int(reply) << endl;
    
    argout = CORBA::string_dup(&reply);
    
	/*----- PROTECTED REGION END -----*/	//	Socket::read_char
	return argout;
}
